...
use diagnostics;
use Getopt::Long;
#our $SAMTOOLSEXE = '/programs/samtools-1.3/samtools';
use SNPiR::config;
#use File::Spec;
#use File::Basename;
#use Data::Dumper;
...
my ($INPUTFILE,$OUTPUTFILE,$BAMFILE,$REFERENCEGENOME,$HELP,$ILLUMINAQUALS,$PBLAT_PATH);
my $MINBASEQUAL = 25;
my $MINMISMATCH = 1;
my $SCORELIMIT = 0.95;
my $QUALOFFSET = 33;
...
sub parse_command_line {
	...
	if(!$INPUTFILE || !$OUTPUTFILE || !$BAMFILE || !$REFERENCEGENOME || !$PBLAT_PATH){
	    usage();
	}
	if($ILLUMINAQUALS){
		$QUALOFFSET = 64;
...
my $pslfile = join '', $INPUTFILE, '.psltmp';
	...
	print STDERR "blatting reads (threads: $THREAD_COUNT)...\n";
